...
class TransactionController extends GetxController {
  ...
  getAccountList() async {
    var response = await httpService.getRequest('transactions/');
    if (response.isOk) {
  ...
  Future<Response> createTransaction(
      TransactionFormModel transactionFormModel) async {
    try {
      final response = await httpService.postRequest(
          'transactions/', transactionFormModel.toJson());
      return response;
    } catch (e) {
  ...
  getAccountTypes() async {
    var response =
        await httpService.getRequest('transactions/get_account_type/');
    if (response.isOk) {
      final List<dynamic> jsonList = jsonDecode(response.bodyString ?? '');
